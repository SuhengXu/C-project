...
void Research(pINAEX head)//查找某年某月某日所有收支信息的函数
{
	pINAEX temp;
	float fValue;
	int iYear;
	...
	int iDay;
	int judgement;
	judgement = 0;// 用于判断是否一条记录都没有查到的标志
		...
		do
		{
			if (temp->iYearDate == iYear && temp->iMonthDate == iMonth && temp->iDayDate == iDay)
			{
				wprintf(L"所查项目为第%d项收支金额:%.2f 地点:%ls 类型:%ls 时间%d.%d.%d\n", temp->iNumber, temp->fValue, temp->szLocation, temp->szKind, temp->iYearDate, temp->iMonthDate, temp->iDayDate);
				judgement = 1;// 如果查到了记录，就把变量judgement变量赋值为1
			}
			temp = temp->pNext;
		} while (temp != NULL);
		if(judgement==0)
...
/*int SafeInputCheck(wchar_t wcArray[],int iLimit)//检查输入的字符是否超出数组容量，用1代表超出
{
	if ((wsclen(wcArray)) > (iLimit - 1))//比较数组长度与容量，预留一个字符为空字符
	{
		printf("*************************\n");
		printf("超过字数上限，请重新输入\n");
		printf("*************************\n");
